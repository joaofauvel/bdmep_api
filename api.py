...
def vars(freq, st):
    """
    Returns a list of dictionaries of avaiable variables for the frequency
    and station type. 
    freq is a string and corresponds to the data frequency: hourly, daily or
    monthly ('h', 'd', 'm').
...
def stations(st_type, region=None):
    """
    Returns list of dictionaries, each entry correponding to an avaiable station.
    freq is a string and must be 'h', 'd' or 'm' for hourly, daily and monthly
    frequencies.
    st_type is a string and must be 'automatic' or 'conventional'.
    region is a string and must be 'N', 'NO', 'S', 'SU' or 'CO' for north,
    northeast, south, southeast and middle west.
    """
...
def var_codes(freq, st, variables=None):
    ...
    When variables param is None returns a list containg all variable codes
    for a given frequency. If variables param is a list, returns a list 
    containing codes for the given aliases of descriptions.
    Aliases are only avaiable to automatic station type.
    """
    # {'CODIGO': 'I209', 'TIPO_GERACAO': 'Dado Calculado (Somatório dos valores - Total do Periodo)',
    # 'DESCRICAO': 'PRECIPITACAO TOTAL, MENSAL (AUT)', 'PERIODICIDADE': 'Mensal', 'UNIDADE': 'mm',
    # 'CLASSE': 'Precipitação'}
    freq_types = ['h', 'd', 'm']
    ...
    if freq.lower() == 'h' and st == 'automatic':
        variables_aliases = {
            'rain': 'I175', 'P_mean': 'I106', 'P_max': 'I615',
            'P_min': 'I616', 'T_mean': 'I101', 'T_max': 'I611',
            'T_min': 'I612', 'R_s': 'I133', 'RH_mean': 'I105',
            'RH_max': 'I617', 'RH_min': 'I618', 'U_mean': 'I111',
            'U_max': 'I608',
            }
    elif freq.lower() == 'd' and st == 'automatic':
        variables_aliases = {
            'rain': 'I006', 'P_mean': 'I109','T_mean': 'I104',
            'T_max': 'I007', 'T_min': 'I008', 'RH_mean': 'I120',
            'RH_min': 'I256', 'U_mean': 'I009', 'U_max': 'I621',
            }
    elif freq.lower() == 'm' and st == 'automatic':
        variables_aliases = {
            'days_raining': 'I230', 'rain': 'I209',
            'P_mean': 'I219','T_mean': 'I220',
            'U_mean': 'I218', 'U_max': 'I221',
            }
    else:
        raise Exception("Error. Aliases are only supported with st='automatic'")
    
    if variables is None:
        codes = [code['CODIGO'] for code in vars(freq, st)]
        ...
        for var in variables:
            if var in variables_aliases and st == 'automatic':
                ...
                raise Exception("Error. Variable aliases currently only work for automatic stations.")
            elif re.match(r'I\d{3}', var, flags=re.I):
                # Treat as var code 
                codes.append(var)
            else:
                # Treat as description
                for v in vars(freq, st):
                    if v['DESCRICAO'] == var:
...
def st_codes(st, region=None, sts=None):
    ...
    When stations param is None returns a list contaning all station codes
    for a given station type and region. If stations param is a list, returns 
    a list containing codes for the given city state names. Valid stations are
    a city followed by the state's double char code, such as Sorocaba-SP or
    Coxim/MS or Tres Lagoas MS or Sao Jose do Rio PretoSP. Must be ASCII, but
    case is ignored.
    """
    ...
    stations_request = stations(st, region)
    if sts is None:
        codes = [code['CD_ESTACAO'] for code in stations_request]
    else:
        for station in sts:
            if (match := re.match(r'^([ a-z]+)[^a-z]?([A-Z]{2})$', station, flags=re.I)):
                codes = [station_['CD_ESTACAO'] for station_ in stations_request
                if station_['SG_ESTADO'] == match.group(2).upper() and
                station_['DC_NOME'] == match.group(1).upper()]
            else:
                raise ValueError("Invalid stations. Expected a string formatted as such: city followed by the state double char code (ie Sorocaba SP).")

            return codes


import datetime
def gen_payload(email, freq, st, in_date, fn_date, dec='.', var_sel=None,
                *st_sel):
    """
    ...
    in_date and fn_date: a date formatted as 'YYYY-MM-DD' or
    'since' (exclusive for in_date) or a datetime object.

    Automatic station type:
    Valid variables (var_sel) for hourly frequency are:
    'rain', 'P_mean', 'P_max', 'P_min', 'T_mean', 'T_max', 'T_min',
    'R_s', 'RH_mean', 'RH_max', 'RH_min', 'U_mean', 'U_max',
    any variable description or variable code.
    For daily frequency: 'rain', 'P_mean', 'T_mean', 'T_max', 'T_min',
    'RH_mean', 'RH_min', 'U_mean', 'U_max', variable description
    or variable code.
    For monthly frequency, 'days_raining', 'rain', 'P_mean', 'T_mean',
    'U_mean', 'U_max', descriptions or codes are valid.
    The resulting data will be aggregated on the specified frequency.
    ...
    if isinstance(in_date, datetime.datetime):
        ...
        raise ValueError("Invalid initial date type. Expected a datetime object or a %Y-%m-%d formatted string.")
    ...
    if isinstance(fn_date, datetime.datetime):
        ...
        raise ValueError("Invalid final date type. Expected a datetime object or a %Y-%m-%d formatted string.")
...
payload = gen_payload(
    'joao.fauvel@unesp.br','h', 'automatic', '2020-12-01', datetime.date.today(),
    '.', ['rain', 'P_mean', 'T_mean', 'R_s', 'RH_mean', 'U_mean'], 'A713')
